...
<body style="background-color:#eee;">
        <div class="container">
            ...
            <div class="title">
                <img src="images/requests.png" align="center"><br>
                <p>Requests</P>
            </div>
...
$dbc=mysqli_connect('127.0.0.1','root','','test')
    ...
    $query="select * from hiredeal where creator2='{$_SESSION['login_user']}' and status=''";
    ...
    $about=$row1['about'];
    	...
    	$hire.=" <div class=\"divid\">
                ...
                <h1 style=\"margin-top:-20px;\">Duration:{$duration} months</h1>
                <p id=\"gist\">Amount for the duration: {$amount}<br></p>
                
                <p><a id=\"butt\" href=\"accept.php?dealid={$dealid}\">Accept &raquo;</a>
                 <a id=\"butt\" href=\"decline.php?dealid={$dealid}\">Decline &raquo;</a>
                </div>";
    }



    $query="select * from copyrightdeal where creator2='{$_SESSION['login_user']}' and status=''";
    $result=mysqli_query($dbc,$query);
    $buy='';
    while($row=mysqli_fetch_array($result))
    {
