...
$dbc=mysqli_connect('127.0.0.1','root','','test')
 ...
 $query="select * from creator where creatorid={$of}";
    ...
    $about1=$row['about'];
$query="select * from follower where creator2={$of}";
    $result=mysqli_query($dbc,$query);
    $output='';
    while($row=mysqli_fetch_array($result))
    {
        $query1="select * from creator where creatorid={$row['creator1']}";
        $result1=mysqli_query($dbc,$query1);
        $row1=mysqli_fetch_array($result1);
        $name="{$row1['fname']} {$row1['lname']}";
        $username=$row1['username'];
        $about=$row1['about'];
        $url=calculateimage1($row['creator1']);
        $output.="<li>
                        <div class=\"ch-item ch-img \" style=\"background-image:url({$url});background-size:contain\">
                            <div class=\"ch-info\">
                                <h3>{$name}</h3>
                                <p style=\"margin: -25px 20px;font-size: 13px; border-top: 2px solid rgba(255,255,255,0.8);\">{$about}<a href=\"profile.php?view={$row['creator1']}\">
                                    @{$username}</a></p>
                            </div>
                        </div>
                    </li>";
